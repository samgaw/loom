defmodule LoomAworsetTest do
  ...
  doctest Loom.CRDT.Loom.AWORSet
  ...
  test "Disjoint adds + a remove" do
    ...
    # Make sure we don't get a resurrected 2.
    setB3 = Set.join(setB2, deltaA)
  ...
  test "Out of order delta application" do
    ...
    assert Enum.sort(CRDT.value(set3)) ==
             Set.join(d1, d3) |> Set.join(d2) |> CRDT.value() |> Enum.sort()
  end
