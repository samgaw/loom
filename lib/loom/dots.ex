defmodule Loom.Dots do
  ...
  def init(
        {%Dots{dots: d, ctx: ctx, initial_counter: initial_counter} = dots, delta_dots},
        actor,
        value
      ) do
  ...
  def add(
    ...
    # What's the value of our clock?
    clock = max(Map.get(ctx, actor, 0), initial_counter) + 1
    dot = {actor, clock}
  ...
  def remove(
        {%Dots{dots: d, initial_counter: initial_counter} = dots,
         %Dots{dots: delta_d} = delta_dots},
        pred
      )
    ...
    {new_delta_d, delta_dots_cloud} =
      Enum.reduce(delta_d, {%{}, []}, fn {dot, v}, {d, cloud} ->
        if pred.(v) do
          # Don't reinsert dot/value, add dot to cloud for causation
          {d, [dot | cloud]}
        else
    ...
    new_dots =
      %Dots{dots | dots: new_d, cloud: dots_cloud, initial_counter: initial_counter}
      |> compact
    ...
    new_delta =
      %Dots{
        delta_dots
        | dots: new_delta_d,
          cloud: dots_cloud ++ delta_dots_cloud,
          initial_counter: initial_counter
      }
      |> compact
  ...
  def empty({%Dots{cloud: cloud, initial_counter: initial_counter} = dots, delta_dots}) do
    {%Dots{dots | dots: %{}},
     join(%Dots{cloud: cloud, initial_counter: initial_counter}, delta_dots)}
  end
  ...
  def remove({%Dots{dots: d, initial_counter: initial_counter} = dots, %Dots{} = delta}) do
    ...
    new_delta = join(delta, %Dots{cloud: Map.keys(d), initial_counter: initial_counter})
    {new_dots, new_delta}
  end
  ...
  defp compact_reduce([{actor, clock} = dot | cloud], ctx, inc_initial_counter, cloud_acc) do
    case {ctx[actor], clock} do
      {nil, ^inc_initial_counter} ->
        compact_reduce(cloud, Map.put(ctx, actor, clock), inc_initial_counter, cloud_acc)
  ...
  defp do_join(
         %Dots{dots: d1, ctx: ctx1, cloud: c1, initial_counter: initial_counter} = dots1,
    ...
    keys1_ahead_ctx2 =
      keys1 |> Enum.filter(fn {actor, clock} -> clock > Map.get(ctx2, actor, 0) end)

    keys2_ahead_ctx1 =
      keys2 |> Enum.filter(fn {actor, clock} -> clock > Map.get(ctx1, actor, 0) end)
